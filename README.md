# 🏙️ GentriMap — Sistema de Alerta Precoce de Gentrificação no Rio de Janeiro
...
> **Um pipeline de ciência de dados espacial, totalmente automatizado, que detecta pressão de deslocamento urbano antes que ela se torne irreversível** — combinando scraping de aluguéis em tempo real, modelagem de contágio espacial e ensemble de ML para gerar, mensalmente, scores de risco de gentrificação para cada bairro do Rio de Janeiro.
...
│   └── GRÁFICO_DE_CALOR.py          # Geocodifica bairros via Nominatim → COORDENADAS.csv
│
└── data/
...
```
[Coleta] → [Tratamento e Features] → [Geocodificação] → [Modelo ML] → [Exportação SQL]
```
...
**Índice de Pressão** — razão entre o preço por m² e o número de unidades residenciais, operacionalizando a *rent gap* de Smith na escala do bairro: onde a oferta é restrita e os preços sobem, o diferencial entre o valor atual e o valor potencial do solo está se abrindo ativamente.
...
| | Recurso | Link |
|---|---|---|
| 📄 | Artigo Técnico Completo | [*Predicting Urban Displacement Before It Happens*](https://github.com/LauroGobatto/PROJETO_GENTRIFICACAO_RIO) |
